/* =========================================================
 * host_studio.js (v103: GameType Support)
 * =======================================================*/
...
App.Studio = {
    ...
    // ... (startRoom, enterHostMode, loadProgramList, renderTimeline は v101/v102と同じ) ...
    // ※ startRoom での this.panelState = Array(25).fill(0); のリセットは必須
    ...
    // ... (途中省略: loadProgramList, renderTimeline, setStep などは v101/v102 を維持) ...
    // ... setupPeriod だけ以下のように変更 ...
    ...
    // ... (renderPanelControl, setPanelColor は v102 と同じ) ...
    ...
    // ※ setStep, goNext, renderMonitorMessage など、その他の関数は前回の v99/101 をそのまま使用してください。
    // ※ 重要なのは setupPeriod の条件分岐変更です。
    // ... (残りの関数群) ...
    ...
    // ... (renderMonitorMessage, renderQuestionMonitor, checkBuzz, judgeBuzz, judgeSolo, judgeSimultaneous, toggleAns, translateMode, quickStart はそのまま) ...
    renderMonitorMessage: function(label, text) {
        document.getElementById('studio-q-text').innerHTML = `<div style="display:flex; justify-content:center; align-items:center; height:200px;"><div style="font-size:2.5em; color:#ffd700; font-weight:bold; text-shadow:0 0 10px rgba(0,0,0,0.5);">${text}</div></div>`;
    ...
    enterHostMode: function(isQuick) {
        ...
        targets.forEach(id => {
            ...
            if(el) {
                el.textContent = code;
                el.onclick = () => {
                    navigator.clipboard.writeText(code).then(() => { App.Ui.showToast("📋 部屋IDをコピーしました！"); });
                };
            }
        });
