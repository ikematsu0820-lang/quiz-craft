/* =========================================================
 * host_config.js (v56: Custom Time & Locked UI)
 * =======================================================*/
...
function enterConfigMode() {
    ...
    select.onchange = () => {
        ...
        if(val) {
            ...
            // ★v56: カスタムエリアを最初から表示する仕様変更（時間設定のため）
            toggleCustomScoreArea(true); // 強制表示
    ...
    const bulkTimeBtn = document.getElementById('config-bulk-time-btn'); // ★v56
    ...
    if(bulkTimeBtn) { // ★v56
        bulkTimeBtn.onclick = () => {
            ...
            document.querySelectorAll('.q-time-input').forEach(input => input.value = val);
        };
    }
...
function toggleCustomScoreArea(forceShow = false) {
    ...
    if (forceShow || area.classList.contains('hidden')) {
        ...
        selectedSetQuestions.forEach((q, i) => {
            ...
            div.innerHTML = `
                <div style="flex:1; overflow:hidden; white-space:nowrap; text-overflow:ellipsis; font-weight:bold; font-size:0.9em;">
                    Q${i+1}. ${q.q}
                </div>
                <div style="display:flex; align-items:center; gap:3px;">
                    ...
                    <span style="font-size:0.7em; color:#0055ff; margin-left:5px;">Pt</span>
                    ...
                    <span style="font-size:0.7em; color:#d00; margin-left:5px;">Loss</span>
...
function addPeriodToPlaylist() {
    ...
    const newConfig = {
        ...
        // ★v56: 全体制限時間は使わず、各問題の設定を優先するが、一応デフォルト0で保持
        timeLimit: 0, 
