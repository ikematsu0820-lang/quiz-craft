...
function addPeriodToPlaylist() {
    ...
    let taSeconds = 5;
    if (mode === 'time_attack') {
        taSeconds = parseInt(getElVal('config-ta-seconds', 5));
    }
    const soloStyle = getElVal('config-solo-style', 'manual');
    if (mode === 'solo' && soloStyle === 'auto') {
         taSeconds = parseInt(getElVal('config-solo-seconds', 5));
    }
    ...
    const newConfig = {
        ...
        buzzTime: parseInt(getElVal('config-buzz-timer', 0)),
